...
<#if user??>
    <h1>Welcome to University Student System dear ${user.firstName!"teacher"}!</h1>
    <#if userCourses??>
        <h3><u>Retrieved Courses</u></h3>
        <#if semester == 1>
            <form action="/teacher/semester/${semesterPlusOne}" method="GET">
                <button value="Next Semester" type="submit" class="btn btn-success">
                    <span class="glyphicon glyphicon-chevron-right"></span>
                </button>
            </form>
        <#elseif semester == 8>
            <form action="/teacher/semester/${semesterMinusOne}" method="GET">
